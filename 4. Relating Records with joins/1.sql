...
VALUES ('Nicole');
--
-- Contoh: JOIN 1 tabel (users + photos) — TANPA ORDER BY
-- Ambil daftar user beserta foto mereka (hanya yang punya foto)
SELECT
  users.id AS user_id,
  users.username,
...
--
-- Contoh: JOIN 2 tabel (users + photos + comments) — TANPA ORDER BY
-- Ambil user, foto mereka, dan komentar pada foto tersebut
SELECT
  u.id AS user_id,
  u.username,
  p.id AS photo_id,
...
JOIN comments c ON c.photo_id = p.id AND u.id = c.user_id;
